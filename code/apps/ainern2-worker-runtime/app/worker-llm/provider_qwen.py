...
class QwenProvider(BaseWorker):
    """Call Qwen (通义千问) API and return structured result."""
    ...
    async def execute(self, job_payload: dict) -> WorkerResult:
        ...
        try:
            ...
            model_id: str = job_payload.get("model_id", "qwen-max")
            ...
            # TODO: call real Qwen API via dashscope SDK
            logger.info(
            ...
            output_text = f"[Qwen stub] model={model_id}, prompt_len={len(prompt)}"
            token_usage = {
