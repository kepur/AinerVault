...
from ainern2d_shared.schemas.skills.skill_19 import Skill19Input, Skill19Output
...
class ComputeBudgetService(BaseSkillService[Skill19Input, Skill19Output]):
    ...
    def execute(self, input_dto: Skill19Input, ctx: SkillContext) -> Skill19Output:
        """
        核心执行逻辑 — TODO: 根据 SKILL_19_*.md 规格实现。

        实现要点:
        1. 读取 SKILL_19_*.md 的 Branching Logic 和 State Machine
        2. 按状态机逐步推进
        3. 调用 modules/ 下的底层模块
