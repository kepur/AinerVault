...
from ainern2d_shared.schemas.skills.skill_13 import Skill13Input, Skill13Output
...
class FeedbackLoopService(BaseSkillService[Skill13Input, Skill13Output]):
    ...
    def execute(self, input_dto: Skill13Input, ctx: SkillContext) -> Skill13Output:
        """
        核心执行逻辑 — TODO: 根据 SKILL_13_*.md 规格实现。

        实现要点:
        1. 读取 SKILL_13_*.md 的 Branching Logic 和 State Machine
        2. 按状态机逐步推进
        3. 调用 modules/ 下的底层模块
