...
from ainern2d_shared.schemas.skills.skill_05 import Skill05Input, Skill05Output
...
class AudioAssetPlanService(BaseSkillService[Skill05Input, Skill05Output]):
    ...
    def execute(self, input_dto: Skill05Input, ctx: SkillContext) -> Skill05Output:
        """
        核心执行逻辑 — TODO: 根据 SKILL_05_*.md 规格实现。

        实现要点:
        1. 读取 SKILL_05_*.md 的 Branching Logic 和 State Machine
        2. 按状态机逐步推进
        3. 调用 modules/ 下的底层模块
