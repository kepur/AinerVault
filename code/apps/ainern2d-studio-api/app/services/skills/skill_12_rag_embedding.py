...
from ainern2d_shared.schemas.skills.skill_12 import Skill12Input, Skill12Output
...
class RagEmbeddingService(BaseSkillService[Skill12Input, Skill12Output]):
    ...
    def execute(self, input_dto: Skill12Input, ctx: SkillContext) -> Skill12Output:
        """
        核心执行逻辑 — TODO: 根据 SKILL_12_*.md 规格实现。

        实现要点:
        1. 读取 SKILL_12_*.md 的 Branching Logic 和 State Machine
        2. 按状态机逐步推进
        3. 调用 modules/ 下的底层模块
