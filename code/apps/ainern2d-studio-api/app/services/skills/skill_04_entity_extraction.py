...
from ainern2d_shared.schemas.skills.skill_04 import Skill04Input, Skill04Output
...
class EntityExtractionService(BaseSkillService[Skill04Input, Skill04Output]):
    ...
    def execute(self, input_dto: Skill04Input, ctx: SkillContext) -> Skill04Output:
        """
        核心执行逻辑 — TODO: 根据 SKILL_04_*.md 规格实现。

        实现要点:
        1. 读取 SKILL_04_*.md 的 Branching Logic 和 State Machine
        2. 按状态机逐步推进
        3. 调用 modules/ 下的底层模块
