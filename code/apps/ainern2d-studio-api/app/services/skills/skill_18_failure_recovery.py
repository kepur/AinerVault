...
from ainern2d_shared.schemas.skills.skill_18 import Skill18Input, Skill18Output
...
class FailureRecoveryService(BaseSkillService[Skill18Input, Skill18Output]):
    ...
    def execute(self, input_dto: Skill18Input, ctx: SkillContext) -> Skill18Output:
        """
        核心执行逻辑 — TODO: 根据 SKILL_18_*.md 规格实现。

        实现要点:
        1. 读取 SKILL_18_*.md 的 Branching Logic 和 State Machine
        2. 按状态机逐步推进
        3. 调用 modules/ 下的底层模块
