...
from ainern2d_shared.schemas.skills.skill_09 import Skill09Input, Skill09Output
...
class VisualRenderPlanService(BaseSkillService[Skill09Input, Skill09Output]):
    ...
    def execute(self, input_dto: Skill09Input, ctx: SkillContext) -> Skill09Output:
        """
        核心执行逻辑 — TODO: 根据 SKILL_09_*.md 规格实现。

        实现要点:
        1. 读取 SKILL_09_*.md 的 Branching Logic 和 State Machine
        2. 按状态机逐步推进
        3. 调用 modules/ 下的底层模块
