...
from ainern2d_shared.schemas.skills.skill_01 import Skill01Input, Skill01Output
...
class StoryIngestionService(BaseSkillService[Skill01Input, Skill01Output]):
    ...
    def execute(self, input_dto: Skill01Input, ctx: SkillContext) -> Skill01Output:
        """
        核心执行逻辑 — TODO: 根据 SKILL_01_*.md 规格实现。

        实现要点:
        1. 读取 SKILL_01_*.md 的 Branching Logic 和 State Machine
        2. 按状态机逐步推进
        3. 调用 modules/ 下的底层模块
