...
from ainern2d_shared.schemas.skills.skill_02 import Skill02Input, Skill02Output
...
class LanguageContextService(BaseSkillService[Skill02Input, Skill02Output]):
    ...
    def execute(self, input_dto: Skill02Input, ctx: SkillContext) -> Skill02Output:
        """
        核心执行逻辑 — TODO: 根据 SKILL_02_*.md 规格实现。

        实现要点:
        1. 读取 SKILL_02_*.md 的 Branching Logic 和 State Machine
        2. 按状态机逐步推进
        3. 调用 modules/ 下的底层模块
