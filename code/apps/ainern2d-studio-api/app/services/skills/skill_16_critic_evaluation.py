...
from ainern2d_shared.schemas.skills.skill_16 import Skill16Input, Skill16Output
...
class CriticEvaluationService(BaseSkillService[Skill16Input, Skill16Output]):
    ...
    def execute(self, input_dto: Skill16Input, ctx: SkillContext) -> Skill16Output:
        """
        核心执行逻辑 — TODO: 根据 SKILL_16_*.md 规格实现。

        实现要点:
        1. 读取 SKILL_16_*.md 的 Branching Logic 和 State Machine
        2. 按状态机逐步推进
        3. 调用 modules/ 下的底层模块
