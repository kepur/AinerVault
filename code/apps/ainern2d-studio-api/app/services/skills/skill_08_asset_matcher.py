...
from ainern2d_shared.schemas.skills.skill_08 import Skill08Input, Skill08Output
...
class AssetMatcherService(BaseSkillService[Skill08Input, Skill08Output]):
    ...
    def execute(self, input_dto: Skill08Input, ctx: SkillContext) -> Skill08Output:
        """
        核心执行逻辑 — TODO: 根据 SKILL_08_*.md 规格实现。

        实现要点:
        1. 读取 SKILL_08_*.md 的 Branching Logic 和 State Machine
        2. 按状态机逐步推进
        3. 调用 modules/ 下的底层模块
