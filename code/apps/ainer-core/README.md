...
它把小说编译成译本、电影剧本、分镜、八工种制作单和中英双语提示词，
通过 Capability API 生成图像／对白／声景／动态镜头，并可把章节渲染成
有声书 M4A 或电影 MP4。API 不可用时，同一生产包可在手动模式逐轨复制。
...
| P0.5 | 锁定译本 → 无旁白电影剧本 → 分镜 | ✅ |
| P0.6 | 翻译 + 术语一致性 | ✅ |
| P0.5* | 世界观转译（L1–L4 + 防漂移三道闸） | ✅ |
| P1 | 八工种 + 动作物理链 + 首尾帧 + 声景 + 成片 | ✅（真模型逐端点验证中） |
