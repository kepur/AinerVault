...
`world_lexicon.source_profile_id` 与 `scenes.source_profile_id` 已建，
消歧逻辑 `preflight._disambiguate` 已测。但**没有管线去填 scenes.source_profile_id**——
需要一步「按场景判断属于哪个源圈层」的抽取（穿越小说的切换点就是场景切换）。
